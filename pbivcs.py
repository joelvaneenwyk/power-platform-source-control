...
def _find_confs(path):
    ...
    splat = tuple(i for i in os.path.split(
        os.path.abspath(os.path.normpath(path))) if i)
    confs = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-x', action='store_true', dest="extract", default=True,
                        help="extract pbit at INPUT to VCS-friendly format at OUTPUT")
    parser.add_argument('-c', action='store_false', dest="extract", default=True,
                        help="compress VCS-friendly format at INPUT to pbit at OUTPUT")
    parser.add_argument('-s', action='store_true', dest="textconv", default=False,
